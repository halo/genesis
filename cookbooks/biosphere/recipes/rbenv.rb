homebrew_bin_path = Pathname.new File.join(ENV['BIOSPHERE_SPHERE_PATH'], 'homebrew/bin')
rbenv_root        = Pathname.new File.join(ENV['BIOSPHERE_SPHERE_PATH'], 'rbenv')
rbenv_executable  = homebrew_bin_path.join 'rbenv'

logg %{Ensuring rbenv root directory...}
directory rbenv_root.to_s

node[:biosphere][:rbenv][:rubies].each do |version|

  ruby_bin_path = rbenv_root.join('versions', version, 'bin')
